		...
		for i in range( 9, dataPerMonth, stride ):
			...
			for featureIndex in range( 0, datafeatureNum ):
				...
				for x in range( 0, 10 ):
					...
					if x == 8 : 
						d.append( float(list[featureIndex][idx-(9-x)])**2 )
...
def changeDataType( it, count, yHead ):
	...
	trainData = ones(shape = ( count, featureNum*10+1 ) )
...
def featureNormalization(it):
	...
	for x in range( 0, featureNum*10, 10 ):
		m = mean(it[ : , x : x+10 ])
		s = std(it[ :, x: x+10 ])
		mean_r.append(m)
		std_r.append(s)
		it_norm[ : , x : x+10 ] = ( it_norm[ :, x : x+10 ] - m ) / s
...
def testDataNormalization( testData, mean_r, std_r ):
	...
	for x in range( 0, featureNum*10, 10 ):
		testData[ :, x : x+10 ] = ( testData[ :, x : x+10 ] - mean_r[floor(x/10)] ) /std_r[floor(x/10)]
...
theta = zeros( shape = ( 10*featureNum+1, 1 ) )
